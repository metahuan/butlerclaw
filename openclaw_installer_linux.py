...
# -*- coding: utf-8 -*-
"""
OpenClaw Linux 自动安装程序
功能与 Windows/macOS 版一致，适配 Linux：
1. 检查/安装 Node.js（官方预编译 tar.xz，解压到 ~/.local，无需 sudo）
...
if sys.platform != "linux":
    print("此程序仅支持 Linux。Windows 请使用 openclaw_installer_v2.py，macOS 请使用 openclaw_installer_macos.py")
    sys.exit(1)
...
try:
    ...
    print("错误：需要 tkinter。请安装：Ubuntu/Debian: sudo apt install python3-tk  |  Fedora: sudo dnf install python3-tkinter")
    sys.exit(1)
...
# Node 官方 Linux 预编译包：按架构选择
def _node_linux_arch():
...
NODE_ARCH = _node_linux_arch()
NODE_TAR_NAME = f"node-v{NODEJS_VERSION}-linux-{NODE_ARCH}"
NODE_TAR_URL = f"https://nodejs.org/dist/v{NODEJS_VERSION}/{NODE_TAR_NAME}.tar.xz"
...
def _ensure_path(extra_bin=None):
    """确保 PATH 包含 Linux 常见路径及可选 Node 本地安装路径"""
    paths = ["/usr/local/bin", "/usr/bin", os.path.expanduser("~/.local/bin")]
    if extra_bin:
        paths.insert(0, extra_bin)
...
def _font(name_fallback="DejaVu Sans", size=11):
    """Linux 常用字体"""
...
class OpenClawInstaller:
    def __init__(self, root, config_only=False, dialog_parent=None):
        ...
        self.root.title("OpenClaw 自动安装程序 (Linux)")
        self.root.geometry("750x650")
        self.root.resizable(True, True)
        self.node_local_bin = None  # 若本机安装 Node，则为 ~/.local/node-xxx/bin
        self.center_window()
    ...
    def setup_ui(self):
        font_title = _font("DejaVu Sans", 18)
        font_normal = _font("DejaVu Sans", 10)
        font_small = _font("DejaVu Sans", 9)

        title = tk.Label(self.root, text="OpenClaw 自动安装程序 (Linux)", font=(font_title[0], font_title[1], "bold"))
        title.pack(pady=15)
        subtitle = tk.Label(self.root, text="检查 Node.js → 安装 OpenClaw → 配置模型与 API Key（Node 可安装到 ~/.local，无需 root）", font=font_small, fg="gray")
        subtitle.pack()
        ...
        self.log_text = scrolledtext.ScrolledText(log_frame, height=10, width=80, font=("DejaVu Sans Mono", 9))
    ...
    def check_nodejs(self):
        ...
        if self.node_local_bin:
            node_candidates.insert(0, os.path.join(self.node_local_bin, "node"))
        for node_cmd in node_candidates:
            try:
                cmd = [node_cmd, "--version"] if "/" in node_cmd or node_cmd.startswith(os.path.expanduser("~")) else f"{node_cmd} --version"
    ...
    def install_nodejs(self):
        self.logger.info("=== 安装 Node.js（安装到 ~/.local，无需 root）===")
        self.update_progress(20, "正在下载 Node.js...")
        self.installer_path = os.path.join(tempfile.gettempdir(), f"{NODE_TAR_NAME}.tar.xz")
        try:
            self.download_with_retry(NODE_TAR_URL, self.installer_path, MAX_RETRIES)
        except Exception as e:
        ...
        self.update_progress(35, "正在解压并安装到 ~/.local...")
        local_base = os.path.expanduser("~/.local")
        os.makedirs(local_base, exist_ok=True)
        tmp_dir = tempfile.gettempdir()
        try:
            # tar xJf xxx.tar.xz -C /tmp；包内顶层目录为 node-v20.11.0-linux-x64
            result = subprocess.run(
                ["tar", "xJf", self.installer_path, "-C", tmp_dir],
                capture_output=True, text=True, timeout=60
            )
            if result.returncode != 0:
                raise Exception(result.stderr or "tar 解压失败")
            extracted_top = os.path.join(tmp_dir, NODE_TAR_NAME)
            if not os.path.isdir(extracted_top):
                raise Exception(f"解压后未找到目录: {extracted_top}")
            dest = os.path.join(local_base, NODE_TAR_NAME)
            if os.path.isdir(dest):
                shutil.rmtree(dest)
            shutil.move(extracted_top, dest)
            self.node_local_bin = os.path.join(dest, "bin")
            _ensure_path(self.node_local_bin)
            self.logger.success(f"Node 已安装到: {dest}")
            self.update_progress(50, "Node.js 安装完成")
            # 建议用户将 PATH 写入 shell 配置
            npm_global = os.path.expanduser("~/.npm-global")
            os.makedirs(npm_global, exist_ok=True)
            os.environ["npm_config_prefix"] = npm_global
            if npm_global not in os.environ.get("PATH", ""):
                os.environ["PATH"] = os.path.join(npm_global, "bin") + ":" + os.environ.get("PATH", "")
            # 验证
            node_exe = os.path.join(self.node_local_bin, "node")
            result = subprocess.run([node_exe, "--version"], capture_output=True, text=True, encoding="utf-8", errors="ignore", timeout=10)
            if result.returncode == 0:
                self.logger.success(f"Node.js 版本: {result.stdout.strip()}")
                self.node_installed = True
                return True
            raise Exception("验证 Node 失败")
        except Exception as e:
            self.logger.error(f"安装失败: {e}")
            msg = f"Node.js 安装失败。\n可改用手动安装:\n  sudo apt install nodejs npm  或访问 https://nodejs.org\n\n错误: {e}"
    ...
    def _get_openclaw_candidates_linux(self):
        """Linux 下可能存在的 openclaw 路径（不依赖当前 PATH）"""
        ...
        if self.node_local_bin:
            candidates.insert(0, os.path.join(self.node_local_bin, "openclaw"))
        # nvm / fnm 等
        for pattern in (
            os.path.join(home, ".nvm", "versions", "node", "*", "bin", "openclaw"),
            os.path.join(home, ".local", "share", "fnm", "node-versions", "*", "installation", "bin", "openclaw"),
        ):
            try:
                import glob
    ...
    def check_openclaw(self):
        ...
        def try_run(cmd, use_shell=True, run_env=None):
            try:
                r = subprocess.run(
                    cmd, shell=use_shell, capture_output=True, text=True,
                    encoding="utf-8", errors="ignore", timeout=10, env=run_env or env
                )
        ...
        # 1) 当前 PATH（已加入 ~/.npm-global/bin 等）
        version = try_run("openclaw --version", run_env=env)
        if version:
        ...
        # 2) 显式路径（解决从启动器/桌面启动时 PATH 不完整）
        for path in self._get_openclaw_candidates_linux():
            self.logger.info(f"尝试路径: {path}")
            version = try_run([path, "--version"], use_shell=False, run_env=env)
            if version:
                self.logger.success(f"OpenClaw 已安装: {version}，跳过安装")
                self.openclaw_installed = True
                self.update_progress(95, "OpenClaw 已存在，跳过安装")
        ...
        try:
            r = subprocess.run(
                ...
                shell=True, capture_output=True, text=True, encoding="utf-8", errors="ignore", timeout=15, env=env
            )
    ...
    def _get_current_openclaw_version(self):
        ...
        env = os.environ.copy()
        env.setdefault("PATH", "")
        if os.path.expanduser("~/.npm-global/bin") not in env["PATH"]:
            env["PATH"] = os.path.expanduser("~/.npm-global/bin") + ":" + env["PATH"]
        try:
            r = subprocess.run("openclaw --version", shell=True, capture_output=True, text=True, encoding="utf-8", errors="ignore", timeout=10, env=env)
            if r.returncode == 0 and (r.stdout or "").strip():
                return (r.stdout or "").strip()
        except Exception:
            pass
        for path in self._get_openclaw_candidates_linux():
            try:
                r = subprocess.run([path, "--version"], capture_output=True, text=True, encoding="utf-8", errors="ignore", timeout=10, env=env)
                if r.returncode == 0 and (r.stdout or "").strip():
                    return (r.stdout or "").strip()
            except Exception:
                pass
    ...
    def _get_latest_openclaw_version(self):
        ...
        _ensure_path(self.node_local_bin)
        try:
            r = subprocess.run("npm view openclaw version", shell=True, capture_output=True, text=True, encoding="utf-8", errors="ignore", timeout=15, env=env)
            if r.returncode == 0 and (r.stdout or "").strip():
                return (r.stdout or "").strip()
        except Exception:
            pass
    ...
    def update_openclaw_if_needed(self):
        ...
        env["PATH"] = os.path.join(npm_global, "bin") + ":" + env.get("PATH", "")
        _ensure_path(self.node_local_bin)
        try:
            r = subprocess.run("npm install -g openclaw@latest", shell=True, capture_output=True, text=True, encoding="utf-8", errors="ignore", timeout=300, env=env)
            if r.returncode != 0:
    ...
    def install_openclaw(self):
        ...
        try:
            result = subprocess.run(
                ...
                shell=True, capture_output=True, text=True, encoding="utf-8", errors="ignore", timeout=300, env=env
            )
            if result.returncode != 0:
            ...
            result = subprocess.run(
                "openclaw --version",
                shell=True, capture_output=True, text=True, encoding="utf-8", errors="ignore", timeout=10, env=env
            )
            if result.returncode == 0:
                version = (result.stdout or "").strip()
                ...
                self.logger.info(f"请将 {npm_bin} 加入 PATH，例如在 ~/.bashrc 中添加: export PATH=\"{npm_bin}:$PATH\"")
                return True
        ...
        try:
            ...
            if not self.check_nodejs():
                if not self.install_nodejs():
                    self.root.after(0, lambda: self._show_error("安装失败", "Node.js 未安装成功，请按提示重试或手动安装"))
...
class ConfigDialog:
    ...
    def _setup_ui(self):
        font_title = _font("DejaVu Sans", 16)
        font_n = _font("DejaVu Sans", 11)
        font_s = _font("DejaVu Sans", 9)
        ...
        self.api_key_entry = tk.Entry(api_in, textvariable=self.api_key_var, font=("DejaVu Sans Mono", 10), width=50, show="*")
        ...
        self.api_format_hint = tk.Label(api_frame, text="", font=("DejaVu Sans", 8), fg="gray")
        ...
        self.preview_text = scrolledtext.ScrolledText(preview_frame, height=5, width=70, font=("DejaVu Sans Mono", 9))
